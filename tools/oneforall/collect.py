...
class Collect(object):
    ...
    def get_mod(self):
        ...
        获取要运行的模块
        """
        if Oneforall.enable_all_module:
            ...
            for module in modules:
                module_path = Oneforall.oneforall_module_path.joinpath(module)
                for path in module_path.rglob('*.py'):
                    # 需要导入的类
                    import_module = ('modules.' + module, path.stem)
                    self.modules.append(import_module)
        else:
            self.modules = Oneforall.enable_partial_module
    ...
    def import_func(self):
        ...
        for package, name in self.modules:
            import_object = importlib.import_module('.' + name, 'tools.oneforall.' + package)
    ...
    def run(self):
        ...
        for thread in threads:
            ...
            thread.join(Oneforall.module_thread_timeout)
